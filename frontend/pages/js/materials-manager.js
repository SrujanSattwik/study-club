const API_URL = window.API_CONFIG ? buildApiUrl(API_CONFIG.ENDPOINTS.MATERIALS) : 'http://localhost:3001/api/materials';
...
function renderMaterialsList(materials) {
    ...
    if (materials.length === 0) {
        container.innerHTML = '<p style="text-align: center; padding: 2rem; color: #666;">No materials found in this category yet. Be the first to upload!</p>';
        return;
    }
    ...
    container.innerHTML = materials.map(material => {
        ...
        const resourceUrl = material.link || (material.filePath ? `http://localhost:3001${material.filePath}` : '#');
        ...
        <div class="material-item" data-subject="${material.category || material.type}" data-level="undergraduate" data-title="${escapeHtml(material.title).toLowerCase()}" data-author="${escapeHtml(material.author || 'anonymous')}">
            <div class="material-thumbnail">
                ${material.thumbnail ? `<img src="${material.thumbnail}" alt="${escapeHtml(material.title)}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 8px;">` : `<i class="${iconClass}" style="font-size: 3rem; color: #6366f1;"></i>`}
            </div>
            ...
            <div class="material-actions">
                <button class="btn btn-primary" onclick="handleMaterialDownload('${material.id}', '${resourceUrl.replace(/'/g, "\\'")}')"><i class="fas fa-${material.type === 'video' ? 'play' : material.type === 'audio' ? 'headphones' : 'download'}"></i> ${material.type === 'video' ? 'Watch' : material.type === 'audio' ? 'Listen' : 'Download'}</button>
                <button class="btn btn-secondary" style="background: white; color: #6366f1; border: 1px solid #6366f1;" onclick="saveMaterial('${material.id}')"><i class="fas fa-bookmark"></i> Save</button>
            </div>
        </div>
    `;
    }).join('');
